...
/// Gestore audio: genera a runtime i suoni delle note (toni sintetizzati),
/// senza alcun file esterno né asset protetti da copyright.
///
/// Per ogni classe di nota costruisce un breve WAV PCM in memoria (onda con
/// qualche armonica + inviluppo) e lo riproduce con un pool di player, così le
/// pressioni rapide/simultanee non si tagliano a vicenda.
class AudioManager {
  ...
  /// Costruisce un WAV (PCM 16-bit mono) per un tono alla frequenza data,
  /// con armoniche e inviluppo attacco/decadimento per un suono pulito.
  Uint8List _buildToneWav(double freq) {
    final int n = (_sampleRate * _toneSeconds).round();
    final Int16List samples = Int16List(n);
    const double attack = 0.006; // secondi di salita
    const double tau = 0.16; // costante di decadimento

    for (int i = 0; i < n; i++) {
      final double t = i / _sampleRate;
      // Inviluppo: breve attacco lineare, poi decadimento esponenziale.
      final double env = (t < attack ? t / attack : 1.0) * exp(-t / tau);
      // Fondamentale + 2 armoniche (timbro più ricco).
  ...
  Uint8List _wrapWav(Int16List samples) {
    ...
    o += 4;
    bd.setUint16(o, 1, Endian.little); // PCM
    o += 2;
    bd.setUint16(o, 1, Endian.little); // mono
    o += 2;
    bd.setUint32(o, _sampleRate, Endian.little);
    o += 4;
    bd.setUint32(o, _sampleRate * 2, Endian.little); // byte rate
    o += 4;
    bd.setUint16(o, 2, Endian.little); // block align
    o += 2;
    bd.setUint16(o, 16, Endian.little); // bits per sample
    o += 2;
    str('data');
